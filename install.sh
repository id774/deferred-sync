...
#  Version History:
#  v3.4 2026-09-12
#       Avoid sudo for root, support Solaris cp, and fail on filesystem errors.
#  v3.3 2026-08-23
...
uninstall() {
    ...
    if [ -L /etc/cron.exec/deferred-sync ]; then
        $SUDO rm -vf /etc/cron.exec/deferred-sync || {
            echo "[ERROR] Failed to remove /etc/cron.exec/deferred-sync" >&2
            exit 1
        }
    fi
